...
import com.dong.animation.FrameAnimation;
import com.dong.view.DynamicView;

/**
 * Created by zengwendong on 16/7/13.
 */
public class Sprite {
    ...
    public void setDirection() {
        if (this.x <= 0
                && (this.y + this.height) < DynamicView.SCREEN_HEIGHT) {
            if (this.x < 0)
                this.x = 0;
            this.direction = Sprite.DOWN;
        } else if ((this.y + this.height) >= DynamicView.SCREEN_HEIGHT
                && (this.x + this.width) < DynamicView.SCREEN_WIDTH) {
            if ((this.y + this.height) > DynamicView.SCREEN_HEIGHT)
                this.y = DynamicView.SCREEN_HEIGHT - this.height;
            this.direction = Sprite.RIGHT;
        } else if ((this.x + this.width) >= DynamicView.SCREEN_WIDTH
                && this.y > 0) {
            if ((this.x + this.width) > DynamicView.SCREEN_WIDTH)
                this.x = DynamicView.SCREEN_WIDTH - this.width;
