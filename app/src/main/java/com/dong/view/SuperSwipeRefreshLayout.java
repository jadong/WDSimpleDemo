 ...
 * @Description SuperSwipeRefreshLayout<br>
 * 支持下拉刷新和上拉加载更多<br>
...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    public boolean isChildScrollToTop() {
        ...
        if (android.os.Build.VERSION.SDK_INT < 14) {
            ...
            if (mTarget instanceof FrameLayout) {
                int count = ((FrameLayout) mTarget).getChildCount();
                for (int i = 0; i < count; i++) {
                    View view = ((FrameLayout) mTarget).getChildAt(i);
                    if(view instanceof RecyclerView){
                        isChildScrollToTop = !(((RecyclerView) view).computeVerticalScrollOffset() > 0);
                        break;
                    }
                }

            } else {
                isChildScrollToTop = !ViewCompat.canScrollVertically(mTarget, -1);
            }
    ...
    public interface OnRefreshListener{
        public void onPullToRefresh();//下拉刷新
    }
    /**
     * 设置 下拉刷新监听
     *
